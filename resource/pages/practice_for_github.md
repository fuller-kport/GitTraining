...
~~あっ origin/○○って書くとリモートリポジトリの〇〇ブランチって意味ね~~

A君の送った`add_read_roles`の様にリモートリポジトリに対応するブランチがない
場合でもGitHubが自動で新しくブランチを作ってくれるている．
GitHubを見て`add_read_roles`が出来ていることを確認しよう．

今回君とA君のローカルリポジトリはそれぞれ別のPC上にある設定だ．
だからGitHubの機能を使ってマージする必要がある．
GitHubにはそのプロジェクトの管理者に変更を取り込む事を提案する機能がある，
それがプルリクエスト(pull request)だ! 早速使っていこう!

branchsの所とかに
`New pull request`って書いてあるからそこを押して，後は流れでプルリクエストを作れる．
めっちゃ簡単!

### コンフリクトォ!!
さて，ここからはA君では無くこのプロジェクトの主人である君の作業だ．
