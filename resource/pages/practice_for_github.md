...
## gitで管理したくないファイルはどうする??
管理したく無いファイルの名前を`.gitignore`というファイルに書いておけばいい!
例えば今回生成される実行ファイル`werewolf.out`をGitで管理したくないなら，
`.gitignore`の中身を次の様にすればいい．
```
werewolf.out
```
今回は既に`werewolf.out`がGitの管理対象に入ってしまっているので，それも消したい．
```
git rm [管理対象から外したいファイルやフォルダ]
```
...
ページ上部の`branchs`の所とかに
`New pull request`って書いてあるからそこを押して，後は流れでプルリクエストを作れる．
めっちゃ簡単!
どこからどこにプルリクを出しているかを決めるところがあるので，
`add_read_roles`から`finish`へプルリクを出すようにする所だけ注意すればいい．

### コンフリクトォ!!
ページ上部の`Pull requests`の欄に今送ったプルリクが表示されている．
よく見るとエラーメッセージっぽいものが表示されているはずだ
```
This branch has conflicts that must be resolved
```
どうやらコンフリクトを解決して欲しいらしい…
コンフリクト(conflict)とは衝突等の意味がある．
~~Gitだけではどの変更を優先すればいいのかを判別出来ない状態~~
このままだとマージ出来ないので，Aさんはコンフリクトを解消する必要がある．

今回はAさんプルリクエストの内容を，君の変更と衝突しない内容に書き換えることでコンフリクトを解消する．
まずはカレントブランチを`finish`に変更しよう．
そして次のコマンドでA君のローカルリポジトリの`finish`ブランチに君の変更を取り込む．
```
git pull origin finish
```

`git pull origin [取り込むリモートリポジトリのブランチ]`
これはプッシュの逆… リモートリポジトリの指定したブランチから変更を取得し，
現在のローカルブランチにマージするコマンドだ

それが出来たら`add_read_roles`に移動し，`finish`を`add_read_roles`にマージする．
マージする際にどこがコンフリクトを起こしているかを示すメッセージが出るはずだ．
次はそこを見ながら編集していく．

ファイル内にこんなものが表示されているはずだ
```
<<<<<<< HEAD
//マージ先の変更点
=======
...
最終的に普通のソースコードが出来ればOK．
全部修正出来たら，コミットしてあげるとコンフリクトが解消された事にGitが気づいてマージが完了する．

`add_read_roles`ブランチとfinish間のコンフリクトを解消出来たので早速プッシュしよう．
そしてGitHubのページでプルリクエストを見てみると，コンフリクトを解消しろといっていた部分が
